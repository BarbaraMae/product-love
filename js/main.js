...
fetch("https://dummyjson.com/products")
  ...
  .then((data) => {
    ...
    // Create a list of category buttons to filter products
    // first grab the categories from the api
    ...
    uniqueCategories.forEach((uniqueCategory) => {
      categoryFilters.insertAdjacentHTML(
        "beforeend",
        `
        <li class="bg-pink-300 hover:bg-pink-700 text-black hover:text-white rounded-md px-2 py-2">${uniqueCategory}</li>
      `
      );
    });
